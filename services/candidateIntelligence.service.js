...
export async function getCandidateIntelligenceSummary(filters = {}) {
  ...
  const summary = {
    total: rows.length,
    withWebsite: rows.filter((row) => row.has_website).length,
    withContact: rows.filter((row) => row.has_contact).length,
    verified: rows.filter(
      (row) => String(row.verified_status || "").toLowerCase() === "verified"
    ).length,
    locked: rows.filter((row) => Boolean(row.admin_locked)).length
  };
