...
function detectState(prompt = "") {
  ...
  const namedState = Object.entries(STATE_CODES).find(([name]) =>
    new RegExp(`\\b${name.replace(/ /g, "\\s+")}\\b`, "i").test(normalizedPrompt)
  );
...
function buildStaticPlatformAnswer({ prompt, platformContext }) {
  ...
  const stateFilter = (item) => {
    ...
    return itemStateCode(item) === wantsState;
...
export async function askAiCampaignCopilot({
  ...
  const requestScope = {
    state: normalizeStateCode(payload.state) || detectState(prompt),
    office: clean(payload.office),
    cycle: clean(payload.cycle),
    campaign: clean(payload.campaign || payload.candidate),
    strictGeography:
  ...
  if (requestScope.strictGeography && !requestScope.state) {
    throw new Error(
      "A valid state is required when strict geographic scope is enabled."
    );
  }
   ...
   * Candidate briefing requests are delegated to the Executive

 

   * Intelligence Orchestrator so Executive Chief of Staff uses the

 

   * same verified candidate bundle as the dedicated orchestrator.
   ...
   * All non-candidate prompts continue through the existing Co-Pilot

 

   * platform/LLM flow below.
  ...
  try {
    ...
    const candidateRequestFailed =

      candidateOrchestration ||

      Boolean(clean(payload.candidate || payload.candidate_id)) ||

      /\b(?:complete\s+(?:detailed\s+)?briefing|detailed\s+briefing|comprehensive\s+briefing|full\s+briefing|candidate\s+briefing|brief\s+me)\s+(?:on|about|for|of)\s+[a-z]/i.test(
    ...
    candidateOrchestration = candidateRequestFailed;

 

    orchestratorResult = candidateRequestFailed

      ? {

          answer:

            "Verified candidate intelligence could not be loaded. No finance figures, polling percentages, news claims, political signals, or strategy recommendations will be invented. Review the Executive Intelligence Orchestrator diagnostics and retry.",
  ...
  let resolvedAnswerType = classification.answerType;
  ...
  if (
    ...
    const orchestratorSources =
      ...
      Array.isArray(orchestratorResult.sources)
        ...
        : Array.isArray(
            ...
            orchestratorResult?.briefing?.sources
    ...
    generated = {
      ...
      confidence: Number(
        ...
        orchestratorResult.confidence ??
          ...
          orchestratorResult?.briefing?.confidence ??
      ...
      orchestrator: {
        ...
        coverage:
          ...
          orchestratorResult?.execution?.coverage ||
      ...
      if (llmAnswer) {
        ...
        generated = {
          ...
          confidence:
            ...
            classification.needsLiveResearch
      ...
      platformContext
        ...
        ? {
            ...
            mission_summary:
              ...
              platformContext.mission?.summary ||
            ...
            advisor_summary:
              ...
              platformContext.advisor?.summary ||
            ...
            war_room_summary:
              ...
              platformContext.warRoom?.summary ||
  ...
  return {
    ...
    orchestrator_tool_plan:
      ...
      generated?.orchestrator?.tool_plan ||
    ...
    live_research_connected:
      ...
      candidateOrchestration
        ...
        ? Number(
            ...
            generated?.orchestrator?.coverage?.useful_tools || 0
...
export async function listAiCampaignCopilotThreads({
...
export async function getAiCampaignCopilotThread({
