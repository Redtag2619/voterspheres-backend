...
const POLLING_STATE_NAME_TO_CODE = Object.freeze({
...
const POLLING_STATE_CODE_TO_NAME = Object.freeze(

  Object.fromEntries(

    Object.entries(POLLING_STATE_NAME_TO_CODE).map(([name, code]) => [code, name])
...
function normalizePollingState(value = "") {

  const normalized = clean(value).replace(/\s+/g, " ").toUpperCase();

  if (!normalized) return { input: null, code: null, name: null };

 

  if (/^[A-Z]{2}$/.test(normalized) && POLLING_STATE_CODE_TO_NAME[normalized]) {
    ...
    return {
      ...
      name: POLLING_STATE_CODE_TO_NAME[normalized],
  ...
  const code = POLLING_STATE_NAME_TO_CODE[normalized] || null;
  ...
  return {
    ...
    name: code ? POLLING_STATE_CODE_TO_NAME[code] : normalized,
...
function normalizePollingArgs(args = {}) {

  const normalized = { ...(args || {}) };

  const state = normalizePollingState(args.state_code || args.state);

 

  if (state.code) {

    normalized.state = state.code;

    normalized.state_code = state.code;
