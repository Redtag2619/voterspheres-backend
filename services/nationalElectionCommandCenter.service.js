...
function canonicalKey(item = {}) {
  ...
  if (title) {
    ...
    return ["title", title, state || "national", workspace || "portfolio"].join("|");
...
function semanticRoute(item = {}) {
  ...
  if (text.includes("relationship") || text.includes("network") || text.includes("influence graph")) {
  ...
  if (text.includes("dark money") || text.includes("fec") || text.includes("pac")) {
  ...
  if (text.includes("consultant")) return "/consultant-intel";

 

  if (text.includes("vendor") || text.includes("media buy") || text.includes("direct mail")) {
  ...
  if (text.includes("fundraising") || text.includes("donor") || text.includes("finance")) {
  ...
  if (text.includes("coalition") || text.includes("strategy") || text.includes("path to victory")) {
  ...
  if (text.includes("battleground") || text.includes("race pressure") || text.includes("national race")) {
  ...
  if (text.includes("candidate") || text.includes("opponent")) return "/candidates";
  ...
  if (lower(item.type).includes("crm") || lower(item.category).includes("crm")) {
...
function normalizeMissionItem(item = {}, sequence = 0) {
  ...
  return {
    ...
    description:
      ...
      clean(item.description) || clean(item.summary) || "Executive action requires review.",
  ...
  const normalized = {
    ...
    threat_enrichment: {
      ...
      recommendation:
        ...
        clean(item.recommendation) || clean(item.action) || clean(item.description),
...
function normalizeAdvisorRecommendation(item = {}, sequence = 0) {
  ...
  const normalized = {
    ...
    sources: sourceList(item.sources, item.source, "AI Strategic Advisor"),
...
function mergeCommandItem(existing, incoming) {
  ...
  const existingIsMission = existing.authoritative_origin === "Executive Mission Control";

  const incomingIsMission = incoming.authoritative_origin === "Executive Mission Control";
  ...
  const merged = {
    ...
    description: clean(primary.description) || clean(secondary.description),

    action: clean(primary.action) || clean(secondary.action),

    sources: sourceList(primary.sources, secondary.sources, primary.source, secondary.source),

    origin_ids: idList(primary.origin_ids, secondary.origin_ids, primary.id, secondary.id),

    enriched_by: sourceList(primary.enriched_by, secondary.enriched_by),

    threat_enrichment: primary.threat_enrichment || secondary.threat_enrichment || null,

    advisor_enrichment: primary.advisor_enrichment || secondary.advisor_enrichment || null,
...
export async function getNationalElectionCommandCenter({ user = {} }) {
  ...
  return {
    ...
    summary: {
      ...
      active_clients: clients.filter((client) => lower(client.status) === "active").length,
      ...
      raw_command_candidates: missionItems.length + threats.length + recommendations.length,

      duplicates_collapsed:

        missionItems.length + threats.length + recommendations.length - command_items.length,
