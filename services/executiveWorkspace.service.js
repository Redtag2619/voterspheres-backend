...
export async function getExecutiveWorkspaceDashboard({ user = {}, workspaceId = null }) {
  ...
  const taskParams = selectedId ? [firmId, selectedId] : [firmId];
  const taskSql = selectedId
    ? `
  ...
  const signalParams = selectedId ? [firmId, selectedId] : state ? [firmId, state] : [firmId];
  const signalSql = selectedId
    ? `
    ...
    ? `
      SELECT id, title, summary, state, signal_type, risk, severity, signal_score, workspace_id, created_at
      FROM political_signals
      WHERE firm_id = $1 AND (state = $2 OR state IS NULL)
      ORDER BY signal_score DESC NULLS LAST, created_at DESC NULLS LAST
      LIMIT 60
    `
    : `
  ...
  const crmParams = selectedId ? [firmId, selectedId] : [firmId];
  const crmSql = selectedId
    ? `
  ...
  const activityParams = selectedId ? [firmId, selectedId] : [firmId];
  const activitySql = selectedId
    ? `
  ...
  const tasks = await safeQuery(taskSql, taskParams);
  const signals = await safeQuery(signalSql, signalParams);
  const contacts = await safeQuery(crmSql, crmParams);
  const activities = await safeQuery(activitySql, activityParams);

  const openTasks = tasks.filter((t) => !["done", "complete", "completed", "resolved"].includes(String(t.status || "").toLowerCase()));
  const criticalSignals = signals.filter((s) => riskTone(s.risk || s.severity || s.signal_score) === "critical");
  const openActivities = activities.filter((a) => !["done", "complete", "completed"].includes(String(a.status || "").toLowerCase()));
  const vendorGaps = vendors.filter((v) => ["high", "thin", "at risk", "critical"].includes(String(v.risk || v.coverage_tier || "").toLowerCase()));
  const atRiskClients = clients.filter((c) => ["at risk", "watch", "critical"].includes(String(c.health_status || "").toLowerCase()));
  ...
  return {
    selected_workspace: selectedWorkspace
      ? {
          ...
          name: selectedWorkspace.name || selectedWorkspace.campaign_name || selectedWorkspace.title || `Workspace ${selectedWorkspace.id}`,
