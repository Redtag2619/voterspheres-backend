...
function normalizePacContribution(row = {}) {
  ...
  // relationship is valid only when OpenFEC supplies a contributor committee
...
async function fetchPacContributionsForCandidate({
  ...
  try {
    ...
    // by recipient committee, and an unsupported/ignored candidate filter can

    // return nationwide transactions and assign the same PACs to unrelated

    // candidates.

    if (!committees.length || !allScheduleARows.length) {

      console.warn(

        `[FEC] No committee-specific PAC contributions found for ${validCandidateId}.`
