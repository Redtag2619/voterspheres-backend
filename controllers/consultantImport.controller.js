...
export async function consultantImportStatus(req, res) {
  try {
    ...
    return res.status(500).json({
      ok: false,
      error: error.message || "Failed to load consultant import status",
    });
  }
}
...
export async function consultantRankings(req, res) {
  try {
    ...
    return res.status(500).json({
      ok: false,
      error: error.message || "Failed to load consultant rankings",
    });
  }
}
...
export async function battlegroundConsultantRankings(req, res) {
  try {
    ...
    return res.status(500).json({
      ok: false,
      error:
        error.message || "Failed to load battleground consultant rankings",
    });
  }
}
...
export async function consultantOverlaps(req, res) {
  try {
    ...
    return res.status(500).json({
      ok: false,
      error: error.message || "Failed to load consultant overlaps",
    });
  }
}
...
export async function oppositionExposure(req, res) {
  try {
    ...
    return res.status(500).json({
      ok: false,
      error: error.message || "Failed to load opposition exposure",
    });
  }
}
...
export async function candidateConsultantRelationships(req, res) {
  try {
    ...
    const result = await getCandidateConsultantRelationships(
      candidateId,
      req.query || {}
    );
    ...
    return res.status(500).json({
      ok: false,
      error:
        error.message || "Failed to load candidate consultant relationships",
    });
  }
}
